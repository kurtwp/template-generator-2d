import { state, inchesToPx, pxToInches } from '../State.js';
import { getShapeSizePx } from './Geometry.js';
...
function drawText(shape) {
    const size = getShapeSizePx(shape, ctx);
    const sizeXPx = size.width;
    const sizeYPx = size.height;
    ...
    ctx.fillText(shape.textContent || 'New Node', 0, 0);
    ctx.restore();
}
...
function drawFocusRing(shape, sizeXPx, sizeYPx) {
    ...
    if(isSelected && !printMediaQuery.matches) {
